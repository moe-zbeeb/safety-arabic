...
def main():
    ...
    if model_p.name.startswith("checkpoint-"):
        ready_file = model_p / ".ready"
        if not ready_file.exists():
            print(f"[eval] ERROR: checkpoint {args.model} has no .ready marker. Refusing to eval.")
            sys.exit(2)
        print(f"[eval] Confirmed ready marker present: {ready_file}")
