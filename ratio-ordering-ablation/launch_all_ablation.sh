...
RUNNER="${SCRIPT_DIR}/run_ratio_ordering_ablation.sh"
DATASETS_DIR="${REPO_ROOT}/Training Corpus/ratio_ordering_ablation"
CURATE_SCRIPT="${REPO_ROOT}/Training Corpus/curate_ratio_ordering_ablation.py"
...
download_model() {
    local name="$1"
    local hf_id="$2"
    local dest="${REPO_ROOT}/models/${name}"
...
if [ -f "${REPO_ROOT}/Arasafe/arasafe_human.jsonl" ]; then
...
if [ -d "${REPO_ROOT}/models/Qwen3Guard-Gen-4B" ]; then
...
for MODEL in "${MODELS[@]}"; do
    ...
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] FINISHED: ${MODEL}  (${MODEL_MINS}m $((MODEL_SECS % 60))s)"
done
...
python3 "${SCRIPT_DIR}/collect_ablation_results.py"
